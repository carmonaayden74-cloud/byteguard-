    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans selection:bg-[#00ff88] selection:text-black">
      ...
      <nav className="flex justify-between items-center p-6 max-w-7xl mx-auto">
        ...
        <div className="space-x-6 text-sm font-medium text-gray-400">
          <Link href="/login" className="hover:text-white transition-colors uppercase">{t('home.login')}</Link>
          <Link href="/herramientas" className="px-4 py-2 bg-[#00ff88] text-black rounded hover:bg-[#00cc6a] transition-colors font-bold uppercase">
            {t('home.dashboard')}
          </Link>
        </div>
      </nav>
      ...
      <main className="flex flex-col items-center justify-center text-center mt-20 px-4">
        ...
        <div className="flex gap-4">
          <Link href="/laboratorio" className="px-8 py-4 bg-[#00ff88] text-black font-bold rounded-lg hover:bg-[#00cc6a] transition-all shadow-[0_0_20px_rgba(0,255,136,0.3)] hover:shadow-[0_0_30px_rgba(0,255,136,0.5)] uppercase">
            {t('home.launch_console')}
          </Link>
          <Link href="/legal" className="px-8 py-4 border border-gray-700 text-gray-300 font-bold rounded-lg hover:border-gray-500 hover:text-white transition-all uppercase">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto mt-32 mb-20">
          <FeatureCard
            icon="🛡️"
            title={t('sidebar.site_auditor')}
            desc={t('tools.site_auditor.desc')}
          />
          <FeatureCard
            icon="📡"
            title={t('sidebar.ip_intel')}
      ...
      <footer className="border-t border-gray-900 py-10 text-center text-gray-600 text-sm">
        <p>&copy; {new Date().getFullYear()} {t('home.footer_rights')}</p>
        <p className="mt-2">{t('home.footer_disclaimer')}</p>
      </footer>
    </div>
  );
}
